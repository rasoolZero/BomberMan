...
Control::Control(RenderWindow &_window, Manager &_manager, Game &_game,
                 Audio &_audio, int _width, int _height,
                 thor::ResourceHolder<Texture, std::string> &_textures)
    : window(_window), game(_game), audio(_audio), width(_width),
      height(_height), textures(_textures),
      manager(_manager), turn_seek{{_window.getSize().x - 40.f, 9.f}} {
  ...
  float const left_offset = (width / 2.0) - (BUTTONS / 2.0) * side_margin -
                            (side_margin / 2) - (BUTTONS / 2.0) * scale;
  for (int i = 0; i < BUTTONS; i++) {
    buttons[i].setSize(Vector2f(scale, scale));
    buttons[i].setPosition(left_offset + i * side_margin + i * scale,
                           side_offset);
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    soundButtons[i].setPosition(side_margin / 2.0 * (i + 1) + scale / 2.0 * i,
                                side_offset + scale / 4.0);
  }
...
void Control::load(int turnCount) {
  ...
  bar_transformable.setPosition(turn_seek.getPosition() +
                                (active_position - inactive_position) * 1.125f);
  if (music) {
...
void Control::update(Time DeltaTime) {
  if (playing && !adjusting) {
    turn_seek.advance(static_cast<double>(DeltaTime.asSeconds()) /
                      game.getTurnTime());
  }
  bool changed = false;
  if (active_area.contains(Mouse::getPosition(window)) && progress < 1.0) {
    ...
    changed = true;
  } else if (!active_area.contains(Mouse::getPosition(window)) &&
             progress > 0.0) {
    progress -= DeltaTime / activation_time;
    changed = true;
  }
  if (changed) {
    ...
    for (int i = 0; i < BUTTONS; i++) {
      button_transformable[i].setPosition(
          (active_position - inactive_position) *
          (1.f - powf(progress, 1 + abs(2 - i) / 2.f)));
...
void Control::manageKey(Event::KeyEvent key, bool released) {
  if (!released) {
    ...
    default:
      ...
      } else if (Keyboard::Key::Numpad0 <= key.code &&
                 key.code <= Keyboard::Key::Numpad9) {
      ...
      for (int i = 0; i < BUTTONS; i++) {
        if (buttons[i].getGlobalBounds().contains(pos)) {
          if (i == 0) { // rewind
            ...
            speed = speed % 3 + 1;
            buttons[4].setTexture(
                &textures["speed_button_" + std::to_string(speed)]);
            game.changeSpeed(speed);
            audio.play(Audio::Click);
...
void Control::updateMouse(Event::MouseMoveEvent mouseMove) {
  if (adjusting) {
    ...
    // assert(turn_seek.getValue() == int(turn_seek.getProgress() *
    // game.getLastTurn()));
    setTurn(turn_seek.getValue());
  } else {
...
void Control::toggleSound(bool sound_selected) {
  if (!sound_selected) {
    music = !music;
    soundButtons[sound_selected].setTexture(music ? &textures["music_on"]
                                                  : &textures["music_off"]);
    if (music)
    ...
    sound = !sound;
    soundButtons[sound_selected].setTexture(sound ? &textures["sound_on"]
                                                  : &textures["sound_off"]);
...
void Control::setPlaying() {
  buttons[2].setTexture(playing ? &textures["pause_button"]
