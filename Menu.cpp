...
Menu::Menu(RenderWindow &window, Color background_color, Font &font,
           Texture *logo_texture, Manager &manager, Audio &audio)
    : window{window}, bg{background_color},
      logo_texture{logo_texture}, manager{manager}, audio{audio},
  ...
  for (int i = 0; i < BUTTONCOUNT; i++) {
    ...
    if (i == selected) {
      if (current_color != selected_button_color) {
        current_color = current_color - Color(color_difference.r * progress,
                                              color_difference.g * progress,
                                              color_difference.b * progress, 0);
      ...
      if (current_color != normal_button_color &&
          (i != play || (i == play && !log_dir.empty()))) {
        current_color = current_color + Color(color_difference.r * progress,
                                              color_difference.g * progress,
                                              color_difference.b * progress, 0);
...
void Menu::manageKey(Event::KeyEvent key, bool released) {
  if (released) {
    if (key.code == Keyboard::Key::Escape && exiting_flag) {
      ...
      selected =
          static_cast<Button>((selected + BUTTONCOUNT - 1) % BUTTONCOUNT);
      if (selected == play && log_dir.empty()) {
        // skip play button if not available
        selected =
            static_cast<Button>((selected + BUTTONCOUNT - 1) % BUTTONCOUNT);
      }
    } else if (key.code == Keyboard::Key::Space ||
  ...
  if (!released) {
    if (mouseButton.button == Mouse::Button::Left) {
      if (selectboxes[choose].contains(mouseButton.x, mouseButton.y)) {
        chooseFile();
      } else if (selectboxes[play].contains(mouseButton.x, mouseButton.y) &&
                 !log_dir.empty()) {
        load();
      }
    }
...
void Menu::updateMouse(Event::MouseMoveEvent mouseMove) {
  for (int i = 0; i < BUTTONCOUNT; i++) {
    if ((i != play || (i == play && !log_dir.empty())) &&
        selectboxes[i].contains(mouseMove.x, mouseMove.y)) {
...
void Menu::chooseFile() {
  char *temp_chr;
  std::string temp_str;
  temp_chr = tinyfd_openFileDialog("select a log file", "", 1, fileTypes,
                                   "JSON files", 0);
