...
Game::Game(RenderWindow &_window, Audio &_audio,
           thor::ResourceHolder<Texture, std::string> &_textures,
           thor::ResourceHolder<Font, int> &_fonts, unsigned _controlHeight)
    : window(_window), audio(_audio), textures(_textures) {
  ...
  for (int i = 0; i < Resources_n::spritesCount; i++) {
    ...
    shapes.insert(
        std::pair<std::string, RectangleShape>(name, RectangleShape()));
    shapes[name].setTexture(&textures[name]);
    animations.insert(std::pair<std::string, thor::FrameAnimation>(
        name, thor::FrameAnimation()));
    int textureEdge = textures[name].getSize().y;
    int frames = textures[name].getSize().x / textureEdge;
    for (int j = 0; j < frames; j++)
      animations[name].addFrame(
          1.f, IntRect(j * textureEdge, 0, textureEdge, textureEdge));
    animators.insert(std::pair<std::string, ANIMATOR>(name, ANIMATOR()));
    animators[name].addAnimation(name, animations[name],
                                 seconds(timeThresholds[1]));
  ...
  for (int i = 0; i < 2; i++) {
    player_info_transformable[i].setPosition(playerInfoLeftOffset,
                                             5 + i * (infoSize.y / 2));
    ...
    heart[i].setPosition(names[i].getPosition() +
                         Vector2f(infoSize.x - spacing - 20, fontSize + 5));
    ...
    upgrades[i].setPosition(names[i].getPosition() +
                            Vector2f(0, /*spacing+*/ fontSize + 10));
    extraHealth[i].setFont(_fonts[0]);
    extraHealth[i].setCharacterSize(fontSize - 1);
    extraHealth[i].setStyle(Text::Bold);
    extraHealth[i].setPosition(
        heart[i].getPosition().x + heart[i].getSize().x * 3 / 4,
        heart[i].getPosition().y + heart[i].getSize().y - fontSize);
  }
...
void Game::load(std::wstring logAddress) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    names[i].setCharacterSize(fontSize);
    while ((names[i].getGlobalBounds().width + playerInfoLeftOffset) >=
               infoSize.x &&
           names[i].getCharacterSize() >= minFontSize)
      names[i].setCharacterSize(names[i].getCharacterSize() - 1);
    truncated[i] = false;
    if ((names[i].getGlobalBounds().width + playerInfoLeftOffset) >=
        infoSize.x) {
  ...
  startPoint.x =
      (window.getSize().x - infoSize.x - scale * columns) / 2.0 + infoSize.x;
  ...
  for (int i = 0; i < columns; i++) {
    for (int j = 0; j < rows; j++) {
      ...
      quad[0].position =
          Vector2f(i * scale + startPoint.x, j * scale + startPoint.y);
      quad[1].position =
          Vector2f((i + 1) * scale + startPoint.x, j * scale + startPoint.y);
      quad[2].position = Vector2f((i + 1) * scale + startPoint.x,
                                  (j + 1) * scale + startPoint.y);
      quad[3].position =
          Vector2f(i * scale + startPoint.x, (j + 1) * scale + startPoint.y);
  ...
  winnerText.setPosition((window.getSize().x) / 2.0, (window.getSize().y) / 2.9);
  winnerText.setFillColor(winnerIndex == 1 ? player2Theme : player1Theme);
  winnerDisplay.setSize(Vector2f(winnerText.getGlobalBounds().width + 20,
                                 winnerText.getGlobalBounds().height * 3.5));
  winnerDisplay.setPosition(
      (window.getSize().x - winnerDisplay.getSize().x) / 2,
      (window.getSize().y - winnerDisplay.getSize().y) / 3);
...
void Game::update(Time DeltaTime) {
  ...
  for (int i = 0; i < darkBackground.getVertexCount(); i++) {
    darkBackground[i].color.a = end_progress >= seconds(1.f)
                                    ? 180
                                    : 180 * cbrt(end_progress.asSeconds());
  }
...
void Game::updateAnimators(Time DeltaTime) {
  Time realDeltaTime =
      seconds(DeltaTime.asSeconds() * timeThresholds[1] / timeThreshold);
  for (int i = 0; i < Resources_n::spritesCount; i++) {
    std::string &name = Resources_n::sprites[i];
...
void Game::updateMouse(Vector2f position, bool F_pressed) {
  for (int i = 0; i < 2; i++) {
    if (names[i].getGlobalBounds().contains(
            player_info_transformable[i].getInverseTransform() * position)) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      ...
      if (has_state(mask, Tile_State::wall)) {
        obstacle.setPosition(j * scale + startPoint.x,
                             i * scale + startPoint.y);
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < columns; j++) {
      ...
      if (has_state(mask, Tile_State::fire)) {
        shapes["fire"].setPosition(j * scale + startPoint.x,
                                   i * scale + startPoint.y);
        target->draw(shapes["fire"]);
      }
      if (has_state(mask, Tile_State::deadzone)) {
        deadzone.setPosition(j * scale + startPoint.x,
                             i * scale + startPoint.y);
...
Vector2f Game::getPlayerPosition(int _turn, int player) {
  ...
  int y;
  if (_turn == -1) {
    x = json_["initial_game_data"][std::string("player_") +
                                   std::to_string(player + 1) +
                                   std::string("_init_y")];
    y = json_["initial_game_data"][std::string("player_") +
                                   std::to_string(player + 1) +
                                   std::string("_init_x")];
  } else {
...
void Game::updatePlayer() {
  for (int i = 0; i < 2; i++) {
    ...
    std::string bombPower = std::to_string(
        int(json_["steps"][turn]["players_data"][i]["bomb_power_level"]));
    std::string trapCount = std::to_string(
        int(json_["steps"][turn]["players_data"][i]["traps_left"]));
    upgrades[i].setString("POWER: " + bombPower + "\nTRAPS:  " + trapCount);

    int part;
    if (health[i] > initialHealth) {
      part = 0;
      extraHealth[i].setString(std::string("+") +
                               std::to_string(health[i] - initialHealth));
    } else {
      part = int(map(health[i], 0, initialHealth, 3, 0));
      extraHealth[i].setString("");
    }
    heart[i].setTextureRect(IntRect(part * heartTextureSize, 0,
                                    heartTextureSize, heartTextureSize));
...
void Game::initPlayerAnimation() {
  const std::string playerAnimations[8] = {
  ...
  for (int j = 0; j < 4; j++) {
    for (int k = 0; k < frames; k++) {
      animations[playerAnimations[j]].addFrame(
          1.f, IntRect(k * edge, j * edge, edge, edge));
    }
    animations[playerAnimations[j + 4]].addFrame(
        1.f, IntRect((frames / 2) * edge, j * edge, edge, edge));
  }
  for (int i = 0; i < 2; i++)
    animators.insert(std::pair<std::string, ANIMATOR>(
        "player" + std::to_string(i + 1), ANIMATOR()));

  for (int i = 0; i < 8; i++) {
    animators["player1"].addAnimation(playerAnimations[i],
                                      animations[playerAnimations[i]],
                                      seconds(timeThresholds[1]));
    animators["player2"].addAnimation(playerAnimations[i],
                                      animations[playerAnimations[i]],
                                      seconds(timeThresholds[1]));
  }
}
