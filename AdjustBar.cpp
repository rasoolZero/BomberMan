...
AdjustBar::AdjustBar(Vector2f size) : size{size}, base{size}, bar{{0, size.y}} {
...
int AdjustBar::getValue() { return (bar.getSize().x + 0.5) / turn_width; }
...
bool AdjustBar::contains(Vector2f point) {
  return base.getGlobalBounds().contains(this->getTransform().getInverse() *
                                         point);
}
