...
Intro::Intro(RenderWindow &window, Color background_color,
    ...
    : window{window}, manager{manager}, audio{audio}, bg{background_color},
      ...
      gap{21, 19}, middle{static_cast<Vector2f>(window.getSize()) / 2.f},
      l_move{{0, -(pieces[A].getWing().y + gap.y / 2)},
             MoveAnimation::Mode::ac_de},
      r_move{{0, pieces[A_flipped].getWing().y + gap.y / 2},
             MoveAnimation::Mode::ac_de},
      l_parallel_move{{-(pieces[left_big].getThickness() + gap.x) * 2, 0},
                      MoveAnimation::Mode::settle},
      r_parallel_move{{(pieces[right_big].getThickness() + gap.x) * 2, 0},
                      MoveAnimation::Mode::settle},
      logo_animation{{0, -middle.y}, MoveAnimation::Mode::uniform},
      l_mask{middle, middle - Vector2f{400, 0}, seconds(1.75),
             Vector2f(pieces[left_big].getWing().x,
                      -pieces[left_big].getWing().y) *
                 1.1f,
             Color::White},
      r_mask{middle, middle + Vector2f{400, 0}, seconds(1.75),
             Vector2f(pieces[right_big].getWing().x,
                      -pieces[right_big].getWing().y) *
                 1.1f,
             Color::White} {
  ...
  logo_animator.addAnimation("logo", thor::refAnimation(logo_animation),
                             seconds(1.5f));
  ...
  pieces[I].setPosition({middle.x - (pieces[A].getWing().x - gap.x / 2),
                         middle.y - pieces[I].getWing().y - gap.y / 2});
  pieces[I_flipped].setPosition(pieces[I].getPosition());
  pieces[I_flipped].flip(middle);

  // pieces[left_big].setPosition(pieces[A].getPosition() -
  // pieces[left_big].getWing() + Vector2f{pieces[A].getThickness() + gap.x, 0});
  pieces[left_big].setPosition({middle.x - pieces[A].getWing().x + gap.x / 2 +
                                    pieces[A].getThickness() -
                                    pieces[left_big].getWing().x,
                                middle.y - pieces[left_big].getWing().y});
  pieces[right_big].setPosition(pieces[left_big].getPosition());
  pieces[right_big].move(
      {(middle.x - pieces[right_big].getPosition().x) * 2 -
           (pieces[right_big].getThickness() + pieces[right_big].getWing().x),
       0});
  /*pieces[right_big].flip(middle);
  pieces[right_big].flip(middle);*/
  // pieces[right_big].setPosition({ middle.x - gap.x, middle.y -
  // pieces[right_big].getWing().y });

  pieces[left_small].setPosition(
      {middle.x - (pieces[left_big].getThickness() * 2 +
                   gap.x * (gap.y / gap.x) + gap.x),
       middle.y - pieces[left_small].getWing().y});
  pieces[right_small].setPosition(pieces[left_small].getPosition());
  pieces[right_small].move(
      {(middle.x - pieces[right_small].getPosition().x) * 2 -
           (pieces[right_small].getThickness() +
            pieces[right_small].getWing().x),
       0});
  ...
  l_parallel_animator.addAnimation("l", thor::refAnimation(l_parallel_move),
                                   seconds(.6f));
  r_parallel_animator.addAnimation("r", thor::refAnimation(r_parallel_move),
                                   seconds(.6f));
...
void Intro::update(Time DeltaTime) {
  /*if (active_piece <= 6) &&
          (!l_parallel_animator.isPlayingAnimation() &&
  !r_parallel_animator.isPlayingAnimation()))
  {
          //ready = false;
  ...
  if (ready) {
     ...
     !r_parallel_animator.isPlayingAnimation()))*/
    {
      ...
      logo_transform.setScale(
          Vector2f(1.f, 1.f) -
          Vector2f(.5f, .5f) *
              static_cast<float>(logo_animation.getProgress()));
      ...
      if (active_piece == 8 &&
          !logo_animator.isPlayingAnimation()) { // end of intro
      ...
      if (!l_animator.isPlayingAnimation() &&
          !r_animator.isPlayingAnimation() /*&& active_piece != 4*/) {
        l_move(pieces[active_piece], 1.0);
        r_move(pieces[active_piece + 1], 1.0);
        setNextAnimation();
        /*if (!l_parallel_animator.isPlayingAnimation() &&
        !r_parallel_animator.isPlayingAnimation()) {
                //setNextVeil();
        }
        if (1 <active_piece && active_piece < 6) {
                pieces[active_piece].setPosition(pieces[active_piece -
        2].getPosition()); pieces[active_piece +
        1].setPosition(pieces[active_piece - 1].getPosition());
        }
        if (active_piece != 4) {
                l_animator.playAnimation("l");
                r_animator.playAnimation("r");
  ...
  if (active_piece <= 6) {
    for (int i = active_piece + (active_piece == 4 ? 3 : 1); i >= 0; i--) {
      ...
      if ((i == active_piece) &&
          (active_piece != 0 || (active_piece == 0 && !A_appeared))) {
  ...
  if (active_piece >= 6) {
    for (int i = 0; i < 6; i++) {
      ...
      logo[i].texCoords = Vector2f((*logo_texture).getSize().x / 2.f,
                                   (*logo_texture).getSize().y / 2.f) +
                          (logo[i].position - middle) * (11.2f / 8.f);
    }
    window.draw(logo_veil, logo_transform.getTransform());
    window.draw(logo, RenderStates(BlendAlpha, logo_transform.getTransform(),
...
#endif // DEBUGGING
    if (key.code == Keyboard::Key::Space || key.code == Keyboard::Key::Escape ||
...
void Intro::setNextVeil() {
  ...
  case 0:
    ...
    if (!A_appeared) {
      top_veil[0].position =
          p.getTransform().transformPoint(p.getPoint(1)) - Vector2f(10, 10);
      top_veil[1].position =
          p.getTransform().transformPoint(p.getPoint(5)) + Vector2f(10, -10);
      top_veil[2].position = {p.getTransform().transformPoint(p.getPoint(0)).x -
                                  (p.getWing().x + 5),
                              middle.y + 2};
      top_veil[3].position = {p.getTransform().transformPoint(p.getPoint(0)).x +
                                  (p.getWing().x + 5),
                              middle.y + 2};
      ...
      for (int i = 0; i < 6; i++) {
        bot_veil[i].position =
            2.f * middle -
            top_veil[i].position; // mirror positions based on middle
      }
    }
    break;
  case 2:
    ...
    bot_veil[0].position =
        p.getTransform().transformPoint(p.getPoint(5)) - Vector2f(0, 5);
    bot_veil[1].position =
        bot_veil[0].position + Vector2f(0, p.getWing().y + 10);
    bot_veil[2].position =
        p.getTransform().transformPoint(p.getPoint(1)) + Vector2f(-2, -5);
    bot_veil[3].position =
        p.getTransform().transformPoint(p.getPoint(0)) + Vector2f(-2, 5);
  ...
  for (int i = 0; i < top_veil.getVertexCount(); i++) {
    top_veil[i].color = bg;
    bot_veil[i].color = bg;
    if ((i > top_veil.getVertexCount() - 3) ||
        (i < 2 && top_veil.getVertexCount() == 8)) {
...
void Intro::setNextAnimation() {
  switch (active_piece) {
  case 0:
    if (pieces[0].getPosition().x <
        window.getSize().x / 2.0 - pieces[A].getWing().x * 2 + 1) {
      l_move.reset({gap.x / 2 + pieces[A].getThickness(), 0},
                   MoveAnimation::Mode::accelerate);
      r_move.reset({-(gap.x / 2 + pieces[A_flipped].getThickness()), 0},
                   MoveAnimation::Mode::accelerate);
      A_appeared = true;
    } else {
      l_move.reset({gap.x + pieces[I].getThickness(), 0},
                   MoveAnimation::Mode::decelerate);
      r_move.reset({-(gap.x + pieces[I_flipped].getThickness()), 0},
                   MoveAnimation::Mode::decelerate);
      active_piece += 2;
    }
    setNextVeil();
    ...
    break;
  case 2:
    l_move.reset({-(gap.x + pieces[left_big].getThickness()), 0},
                 MoveAnimation::Mode::settle);
    r_move.reset({gap.x + pieces[right_big].getThickness(), 0},
                 MoveAnimation::Mode::settle);
    active_piece += 2;
    l_parallel_animator.playAnimation("l");
    r_parallel_animator.playAnimation("r");
