...
MoveAnimation::MoveAnimation(Vector2f relative_destination, Mode movement_mode)
    : destination{relative_destination}, mode{movement_mode} {}

template <class T> void MoveAnimation::operator()(T &object, double progress) {
  if (firstCall) {
    source = object.getPosition();
    firstCall = false;
  }
  // prepare "firstCall" for next animation if the current one is complete:
  // firstCall = progress == 1.0f; //irrelevant in thor2.0(progress is not
  // guaranteed to be 1 on last call), pending thor2.1
  ...
  case Mode::de_ac:
    ...
    break;
  case Mode::settle:
    modifier = -1 * std::abs(1.25 * progress - 1.125) + 1.125;
    break;
  default: // uniform
    modifier = progress;
    break;
...
bool MoveAnimation::is_idle() { return firstCall; }
