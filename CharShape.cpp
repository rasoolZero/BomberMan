#include "CharShape.h"
#include <stdexcept>
CharShape::CharShape(char type, Color color, Vector2f wing_vector,
                     float thickness, Color background_color = Color::Black)
    : type{type}, wing_vector{static_cast<float>(std::abs(wing_vector.x)),
                              static_cast<float>(std::abs(wing_vector.y))},
      thickness{thickness} {
  assign_points();
...
void CharShape::flip(Vector2f origin) {
  ...
  if (vertices[0].y > vertices[1].y) {
          if (type == '<' || type == '>') {
                  Transformable::setPosition(vertices[0] - Vector2f(0,
  wing_vector.y));
          }
          else {
          ...
          if (type == '<' || type == '>') {
                  ...
                  Transformable::setPosition(vertices[0] -
  Vector2f(wing_vector.x, 0));
          }
  }
  this->spline.update();*/
}
...
void CharShape::assign_points() {
  ...
  if (type == '^') {
    ...
    vertices.push_back(
        {wing_vector.x, wing_vector.y * (thickness / wing_vector.x)});
