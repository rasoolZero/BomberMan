...
Mask::Mask(Vector2f source, Vector2f destination, Time duration,
           Vector2f wing_vector, Color color)
    : color{color}, source{source}, movement_vector{destination - source},
      wing_vector{wing_vector}, move{movement_vector,
...
void Mask::setBackTexture(const Texture *back_texture,
                          float back_texture_coefficient) {
  this->back_texture = (Texture *)back_texture;
  this->back_texture_coefficient = back_texture_coefficient;
}

void Mask::setBodyTexture(const Texture *body_texture,
                          float body_texture_coefficient) {
...
void Mask::setPosition(Vector2f position) {
  if (move.getProgress() == 0.0) {
    ...
    Vector2f wing = wing_vector; // *static_cast<float>(((cbrt(2 *
                                 // move.getProgress() - 1) + 1) / 2) - (cbrt(2
                                 // * move.getProgress() - 1.5) / 1.94 + 0.59));
    if (move.getProgress() == 1.f) {
      if (vertices[2].position.x > vertices[3].position.x) {
        ...
        vertices[0].position = vertices[1].position =
            position + Vector2f(wing.x, -wing.y);
    ...
    vertices[3].position =
        source +
        movement_vector *
            static_cast<float>(pow(move.getProgress(), 3 * tail_speed_scale));
    // vertices[3].position = source + movement_vector *
    // static_cast<float>(cbrt(2 * move.getProgress() - 1.5) / 1.94 + 0.59);
    Vector2f difference =
        vertices[2].position - vertices[3].position; // head - tail
    double distance = sqrt(pow(difference.x, 2) + pow(difference.y, 2));
    Transform rebase;
    // rotate the wing vector by degrees equal to degrees between x axis and
    // difference
    ...
    // vertices[0].position = vertices[2].position + rebased_r; //add the
    // rebased wing vector to position of head vertices[4].position =
    // vertices[2].position + rebased_l;
    float travelled = std::abs(position.x - source.x);
    if (std::abs(rebased_l.x) > travelled) {
      vertices[0].position =
          vertices[2].position +
          rebased_r * static_cast<float>(travelled / std::abs(rebased_r.x));
      vertices[4].position =
          vertices[2].position +
          rebased_l * static_cast<float>(travelled / std::abs(rebased_l.x));
    } else {
      vertices[0].position = vertices[2].position + rebased_r;
      vertices[4].position = vertices[2].position + rebased_l;
    }
    if (std::abs(vertices[0].position.x - source.x) >= std::abs(difference.x)) {
      ...
      vertices[1].position =
          vertices[3].position +
          rebased_r * static_cast<float>((travelled - std::abs(difference.x)) /
                                         std::abs(rebased_r.x));
      vertices[5].position =
          vertices[3].position +
          rebased_l * static_cast<float>((travelled - std::abs(difference.x)) /
                                         std::abs(rebased_l.x));
      // vertices[1].position = { vertices[0].position.x -
      // (vertices[0].position.x - source.x), vertices[0].position.y };
      // vertices[5].position = { vertices[4].position.x -
      // (vertices[0].position.x - source.x), vertices[4].position.y };
    }
  ...
  if (body_texture != nullptr) {
    for (int i = 0; i < body.getVertexCount(); i++) {
      ...
      body[i].texCoords =
          static_cast<Vector2f>(body_texture->getSize()) / 2.f -
          (source - body[i].position) * body_texture_coefficient;
    ...
    for (int i = 0; i < body_fill.getVertexCount(); i++) {
      ...
      body_fill[i].texCoords =
          static_cast<Vector2f>(body_texture->getSize()) / 2.f -
          (source - body_fill[i].position) * body_texture_coefficient;
    }
  }
  ...
  if (back_texture != nullptr) {
    for (int i = 0; i < back.getVertexCount(); i++) {
      back[i].texCoords =
          static_cast<Vector2f>(back_texture->getSize()) / 2.f -
          (source - back[i].position) * back_texture_coefficient;
